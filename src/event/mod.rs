...
pub enum Error<V> {
    ...
    /// For the LE Connection Complete event: The address type was not recognized. Includes the
    /// unrecognized byte.
    BadLeAddressType(u8),
...
fn to_le_connection_complete<VE>(payload: &[u8]) -> Result<LeConnectionComplete, Error<VE>> {
    ...
    Ok(LeConnectionComplete {
        ...
        peer_bdaddr: ::to_bdaddr_type(payload[5], bd_addr)
            .map_err(|e| Error::BadLeAddressType(e.0))?,
